...
/*Input.Mouse = {
    ...
    _update: function() {
     ...
     // Returns true if the mouse action with the given name is pressed.
    // Returns false otherwise.
    isPressed: function(name) {
        return this.__pressed[name] !== undefined;
    },

    // Returns true if the mouse action with the given name was just
    // pressed. Returns false otherwise.
    justPressed: function(name) {
        return this.__justPressed[name] !== undefined;
    },

    // Returns true if the mouse action with the given name is being held.
    // Returns false otherwise.
    isHeld: function(name) {
        return this.isPressed(name) && !this.justPressed(name);
    },
    ...
    __onMouseDown: function(e) {
        var name = this.MOUSECODES[e.which];
        this.__pressed[name] = true;
    ...
    __onMouseMove: function(e) {
        this.realX = e.clientX;
        this.realY = e.clientY;
        if(!Display.viewport) {
            this.x = this.realX;
            this.y = this.realY;
            return;
        }
        this.x = e.clientX - Display.viewport.offsetLeft;
        this.y = e.clientY - Display.viewport.offsetTop;
    }
};

// Hook up event handlers.
window.onmousedown = Input.Mouse.__onMouseDown.bind(Input.Mouse);
window.onmouseup = Input.Mouse.__onMouseUp.bind(Input.Mouse);
window.onmousemove = Input.Mouse.__onMouseMove.bind(Input.Mouse);*/